...
class Simulation:
    ...
    BOUNDARY_SIZE = BC = 1
    def __init__(self, L: int, save_every: int = 100, wait_for_n_iters: int = 10): # TODO lepsze dorzucanie dodatkowych globalnych parametrów
        """__init__
    ...
    def run(self, N_iterations: int, density: float = 0.05, filename: str  = None) -> dict:
        ...
        for i in tqdm.trange(N_iterations):
            self.drive(int(density*self.L*self.L))
